/**********************************************************************
 * Premake - monodev_cs.c
 * The MonoDevelop C# target
 *
...
*/
	...
	io_print("  <DeploymentInformation strategy=\"File\">\n");
	io_print("    <excludeFiles />\n");
	io_print("  </DeploymentInformation>\n");
...
static void printFile(const char* file)
{
	...
	if (prj_get_buildaction() != NULL)
	{
		...
		{
			puts("** Warning: MonoDevelop does not support the 'Content' build action");
			action = "Nothing";
		}
		subtype = "Code";
	}
	...
	else
	{
		...
		{
			monodev_warncontent = 1;
			action = "Nothing";
		}
	}

	io_print("    <File name=\"%s%s\" subtype=\"%s\" buildaction=\"%s\" />\n", prefix, file, subtype, action);
}
